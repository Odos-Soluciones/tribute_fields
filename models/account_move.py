...
class AccountMove(models.Model):
    ...
    @api.model
    def get_max_sequence(self, sequence_list):
        max_num = -1
        max_sequence = None
    ...
    def _get_domain_for_fiscal_check(self):
        return [
            ("fiscal_check", "=", True),
    ...
    def onchange_fiscal_check(self):
        ...
        moves = self.env["account.move"].search(self._get_domain_for_fiscal_check())

        control_numbers = moves.mapped("control_number")
        fiscal_correlatives = moves.mapped("fiscal_correlative")

        max_control_number = self.get_max_sequence(control_numbers)
        max_fiscal_correlative = self.get_max_sequence(fiscal_correlatives)
        
        self.control_number = self.get_next_sequence(max_control_number)
        self.fiscal_correlative = self.get_next_sequence(max_fiscal_correlative)
        return
